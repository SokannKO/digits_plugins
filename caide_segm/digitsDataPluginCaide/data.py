...
class DataIngestion(DataIngestionInterface):
    ...
    def itemize_entries(self, stage):
        ...
        if stage == constants.TRAIN_DB or (not self.has_val_folder):
            #feature_image_list = self.make_image_list(self.feature_file)
            feature_image_list = self.text_image_list('feature_file')

            #label_image_list = self.make_image_list(self.label_file)
            label_image_list = self.text_image_list('label_file')
    ...
    def text_image_list(self, list_name):
        list = []
        with open(self.userdata[list_name]) as f:
            lines = f.read().splitlines()
            for line in lines:
                fields = line.split()
                filename = fields[0]
